...
public class ProfileController {
	...
	public ResponseEntity<ApiResponse> createProfile(@RequestBody ProfileRequestDto requestDto,
		...
		ProfileResponseDto responseDto = profileService.createProfile(userDetails.getUsername(), requestDto).getBody();
	 ...
	 */
	@GetMapping
	public ResponseEntity<ApiResponse> getProfileByUserId(@AuthenticationPrincipal UserDetails userDetails) {
		List<ProfileResponseDto> responseDto = profileService.getProfileByUserId(userDetails.getUsername()).getBody();
		ApiResponse response = ApiResponse.builder()
			.msg("프로필 조회 성공")
	...
	public ResponseEntity<ApiResponse> updateProfile(@PathVariable("profileId") Long profileId,
													 ...
													 @AuthenticationPrincipal UserDetails userDetails) {
		ProfileResponseDto responseDto = profileService.updateProfile(profileId, requestDto, userDetails).getBody();
		ApiResponse response = ApiResponse.builder()
			.msg("프로필 수정 성공")
			.statuscode(String.valueOf(HttpStatus.OK.value()))
			.data(responseDto)
